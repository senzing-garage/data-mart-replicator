...
public class SqliteUriTest {
    ...
    public void testDefaultConstructor() {
        ...
        assertNull(uri.getUnusedFile(), "Unused file is not null");
    ...
    public List<Arguments> getConstructParameters() {
        try {
            ...
            result.add(Arguments.of(
                null, null, null, Map.of("foo", "bar")));
            
            result.add(Arguments.of(
                null, null, null, Map.of(MODE_KEY, MEMORY_MODE)));

            result.add(Arguments.of(
                null, null, null, Map.of(MODE_KEY, "rwc")));

            result.add(Arguments.of(
                null, null, 
                File.createTempFile("test-", ".db"), 
                Map.of("bar", "foo")));
    ...
    public void testQueryConstructor(String               unusedUser,
                                     ...
                                     Map<String,String>   queryOptions)
    {
        try {
            ...
            {
                fail("Unexpectedly succeeded with non-memory-mode query options: "
                     + queryOptions);
            }
            ...
            {
                fail("Unexpectedly failed with null or memory-mode query options: "
                     + queryOptions);
            }
    ...
    @ParameterizedTest
    @MethodSource("getConstructParameters")
    public void testFileConstructor(String              unusedUser,
                                    String              unusedPassword,
                                    File                file,
                                    Map<String,String>  queryOptions)
    {
    ...
    @ParameterizedTest
    @MethodSource("getConstructParameters")
    public void testFileQueryConstructor(String             unusedUser,
                                         String             unusedPassword,
                                         File               file,
                                         Map<String,String> queryOptions)
    {
        try {
            ...
            if (queryOptions != null && MEMORY_MODE.equalsIgnoreCase(queryOptions.get(MODE_KEY)))
            {
                ...
                assertEquals(file, uri.getUnusedFile(), "Unused file is not as expected");
            } else {
                assertEquals(false, uri.isMemory(), "uri.isMemory() not as expected");
                assertNull(uri.getUnusedFile(), "Unused file is not null null");
                assertEquals(file, uri.getFile(), "File is not as expected");
            }
    ...
    @ParameterizedTest
    @MethodSource("getConstructParameters")
    public void testProtectedConstructor(String             unusedUser,
                                         String             unusedPassword,
                                         File               file,
                                         Map<String,String> queryOptions)
    {
        try {
            ...
            {
                fail("Unexpectedly succeeded with incomplete user credentials.  "
                     + "unusedUser=[ " + unusedUser + " ], unusedPassword=[ "
                     + unusedPassword + " ]");
            }
            ...
            assertEquals(unusedUser, uri.getUnusedUser(), 
                         "Unused user is not as expected");
            assertEquals(unusedPassword, uri.getUnusedPassword(),
                         "Unused password is not as expected");
            assertEquals(expectedOptions, uri.getQueryOptions(), 
                         "Query options are not as expected");
            ...
            if ((unusedUser == null && unusedPassword == null) 
                || (unusedUser != null && unusedPassword != null))
            {
                fail("Unexpectedly failed with consistent user credentials.  "
                     + "unusedUser=[ " + unusedUser + " ], unusedPassword=[ "
                     + unusedPassword + " ]");
            }
    ...
    @ParameterizedTest
    @MethodSource("getConstructParameters")
    public void testParse(String                unusedUser,
                          String                unusedPassword,
                          File                  file,
                          Map<String,String>    queryOptions)
    {
        ...
        try {
            ...
            if (file == null && queryOptions != null
            ...
            uriText = SqliteUri.SCHEME_PREFIX
                + ((file == null) ? ":memory:" : ("//" + authority + file))
                + queryString;

            String expectedUri = SqliteUri.SCHEME_PREFIX
                + ((file == null) ? ":memory:" : ("//" + authority + file))
                + expectedQuery; 
            ...
            assertEquals(expectedUser, uri.getUnusedUser(),
                         "User is not as expected: " + uriText);
            assertEquals(expectedPassword, uri.getUnusedPassword(),
                         "Password is not as expected: " + uriText);

            boolean expectedMemory = (file == null
                                      || (queryOptions != null 
                                          && MEMORY_MODE.equals(queryOptions.get(MODE_KEY))));
            assertEquals(expectedMemory, uri.isMemory(), 
                         "URI memory flag is not as expected: " + uriText);
            if (expectedMemory) {
                assertEquals(file, uri.getUnusedFile(), 
                             "Unused file is not as expected: " + uriText);
                assertNull(uri.getFile(), "File is not null: " + uriText);
            } else {
                assertEquals(file, uri.getFile(), "File is not as expected: " + uriText);
                assertNull(uri.getUnusedFile(), "Unused file is not null: " + uriText);
            }
            assertEquals(expectedOptions, uri.getQueryOptions(), 
    ...
    @ParameterizedTest
    @MethodSource("getConstructParameters")
    public void testEqualsAndHashWithQuery(String               unusedUser,
                                           String               unusedPassword,
                                           File                 file,
                                           Map<String,String>   queryOptions)
    {
        try {
            ...
            Map<String,String> queryOptions2 = (queryOptions == null) 
                ? new TreeMap<>() : new TreeMap<>(queryOptions);
            queryOptions2.put(TextUtilities.randomAlphabeticText(5),
            ...
            assertNotEquals(uri1.hashCode(), uri2.hashCode(),
                "Objects unexpectedly have the same hash codes");
    ...
    @ParameterizedTest
    @MethodSource("getConstructParameters")
    public void testEqualsAndHashWithFile(String                unusedUser,
                                          String                unusedPassword,
                                          File                  file,
                                          Map<String,String>    queryOptions)
    {
    ...
    @ParameterizedTest
    @MethodSource("getConstructParameters")
    public void testEqualsAndHashWithFileAndQuery(String                unusedUser,
                                                  String                unusedPassword,
                                                  File                  file,
                                                  Map<String,String>    queryOptions)
    {
        try {
            ...
            switch (changeIndex) {
                case 0:
                    ...
                    break;
                case 1:
                    queryOptions2 = (queryOptions == null) 
                                  ? new TreeMap<>() : new TreeMap<>(queryOptions);
    ...
    @ParameterizedTest
    @MethodSource("getConstructParameters")
    public void testEqualsAndHashProtected(String               unusedUser,
                                           String               unusedPassword,
                                           File                 file,
                                           Map<String,String>   queryOptions)
    {
