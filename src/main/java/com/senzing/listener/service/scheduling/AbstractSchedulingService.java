...
public abstract class AbstractSchedulingService implements SchedulingService {
     ...
     * sleep
     * between checks on the locks required for tasks that have been postponed
     * due
     ...
     * The initialization parameter to specify the number of milliseconds to
     * sleep
     * between checking to see if task handling should cease. This timeout is
     * used
     * when there are no postponed tasks due to contention. If not configured
     * then
     ...
     * The initialization parameter to specify the number of milliseconds to
     * delay
     * before attempting to execute a follow-up task. This delay is used to give
     * the
     ...
     * {@linkplain #FOLLOW_UP_TIMEOUT_KEY maximum follow-up deferral time} has
     * been
     ...
     * milliseconds to
     * defer a follow-up task. Once a follow-up task has been deferred this
     * number
     * of milliseconds it will no longer be purposely delayed to wait for
     * additional
     * duplicates to be scheduled and collapsed. This is also the amount of time
     * used to cache a follow-up task from persistent storage before considering
     * the
     * cached version expired and make it available again from persistent
     * storage.
     * If not configured then the value is set to
     * {@link #DEFAULT_FOLLOW_UP_TIMEOUT}. If the value is specified it should
     * be
     ...
     * tasks
     * to retrieve from persistent storage at a time to refill the in-memory
     * cache.
     ...
     * elapsed and after it has elapsed, the in-memory cache should be
     * considered
     * expired. If not configured then the value is to
     * {@link #DEFAULT_FOLLOW_UP_FETCH}. If the value is specified it should be
     * a
     * positive number.
     */
     ...
     * {@link #initLockingService(JsonObject)} to specify the Java class name of
     * the
     ...
     * is
     * not specified. This is the class name for {@link
     ...
     * that
     * represents the configuration for the {@link LockingService} instance
     * created
     * by the default implementation of {@link #initLockingService(JsonObject)}
     * using the {@link #LOCKING_SERVICE_CLASS_KEY} init parameter. If the
     * default
     * implementation of {@link #initLockingService(JsonObject)} is overridden,
     * then
    ...
    public enum Stat implements Statistic {
         ...
         * allow
         * duplicates to be collapsed with it) and after each time a duplicate
         * is found.
         ...
         * called
         * successfully (i.e.: without any exceptions) to handle a task
         * (follow-up or
         * not).
         */
         ...
         * {@link TaskHandler#handleTask(String,Map,int,Scheduler)} has been
         * called
         * unsuccessfully (i.e.: with an exceptions being thrown) to handle a
         * task
         * (follow-up or not).
         */
         ...
         * messages may
         * make the round trip more than once if a failure occurs in processing
         * part or
         * all of the message.
         */
         ...
         * is the
         * number of total non-follow-up tasks handled divided by the number of
         * times
         ...
         * task. This
         * the greatest number of duplicate non-follow-up tasks that were
         * collapsed into
         ...
         * the
         * number of total follow-up tasks handled divided by the number of
         * times
         * {@link TaskHandler#handleTask(String, Map, int, Scheduler)} was
         * called to
         * handle those tasks.
         ...
         * This the
         * greatest number of duplicate follow-up tasks that were collapsed into
         * a
         ...
         * function to
         * Stat.dequeue a task from the internal queue. This includes time waiting
         * for the
         * first task to arrive or the next task to arrive after the previous
         * task has
         * been handled.
         */
         ...
         * lock on
         * the scheduling service in order to call the {@link #dequeueTask()}
         * function.
         */
     ...
     * currently
     * running to prevent more than one call for this object process-wide.
     */
     ...
     * for
     * tasks that have been postponed due to contention. This timeout is used
     * when
     ...
     * handling
     * should cease. This timeout is used when there are no postponed tasks due
     * to
     * contention.
     */
     ...
     * follow-up
     * task. This delay is used to give the opportunity to receive duplicate
     * follow-up tasks that can be collapsed. Whenever a duplicate is collapsed,
     * the
     * delay timer starts over unless the {@link #followUpTimeout} has been
     * reached.
     */
     ...
     * task
     * has been deferred this number of milliseconds it will no longer be
     * delayed to
     * wait for additional duplicates to be scheduled and collapsed.
     */
     ...
     * single
     * retrieval. These retrieved follow-up tasks should be handled within the
     * configured {@link #followUpTimeout}.
     */
     ...
     * in
     * mind that this accounts for collapsed tasks so that the handling of
     * collapsed
     * tasks is only counted once.
     */
     ...
     * that
     * this handling time may have completed multiple collapsed tasks.
     */
    ...
    /**
     * The total number of times an attempt was made to Stat.dequeue a message and
     * one
     * was ready.
     */
    ...
    /**
     * Gets the default postponed timeout with which to Stat.initialize if one is not
     * specified in the initialization configuration via the
     * {@link #POSTPONED_TIMEOUT_KEY} initialization parameter. By default, this
    ...
    /**
     * Gets the default follow-up delay with which to Stat.initialize if one is not
     * specified in the initialization configuration via the
     * {@link #FOLLOW_UP_DELAY_KEY} initialization parameter. By default, this
     ...
     * scheduled
     * and collapsed. It may be delayed because of a lack of resources to handle
     * it.
     *
    ...
    /**
     * Gets the default follow-up timeout with which to Stat.initialize if one is not
     * specified in the initialization configuration via the
     * {@link #FOLLOW_UP_TIMEOUT_KEY} initialization parameter. By default, this
     ...
     * implementation
     * returns twice the {@linkplain #getFollowUpTimeout() follow-up timeout}.
     *
     ...
     * persistent
     * search on a single retrieval. The retrieved follow-up tasks should be
     * handled
     * within the {@linkplain #getFollowUpTimeout() follow-up timeout} and so
     * this
     * number should not be so large that the tasks are not handled or their
     * retrieval is renewed within the allotted time.
     *
    ...
    /**
     * Gets the default follow-up fetch count with which to Stat.initialize if one is
     * not
     * specified in the initialization configuration via the
     ...
     *         task
     *         handling should cease. This timeout is used when there are no
     *         postponed tasks due to contention.
     */
    ...
    /**
     * Gets the default standard timeout with which to Stat.initialize if one is not
     * specified in the initialization configuration via the
     * {@link #STANDARD_TIMEOUT_KEY} initialization parameter. By default, this
     * returns {@link #DEFAULT_STANDARD_TIMEOUT}, but it may be overridden to
     * return
     * something more sensible for a derived implementation.
     *
     ...
     * be
     * locked without blocking and locks those resources. If no such pending
     * task
     * could be found then <code>null</code> is returned.
     *
     ...
     * be
     * processed. If the last time this method was called was less than the
     ...
     * for
     * readiness too frequently. Otherwise, this method will find the least
     * recently
     * postponed {@link ScheduledTask} whose set of affected resources
     * (identified
     * by {@link ResourceKey} instances) are not currently locked. If there are
     * no
     ...
     * postponed
     * tasks. This returns <code>true</code> if and only if there is at least
     * one
     ...
     * available
     * for the follow-up task then it is left on the queue. If the last time
     * this
     * method was called was less than the {@linkplain #getPostponedTimeout()
     * postpone timeout} then this method returns <code>null</code> so that the
     ...
     * should
     * mark the retrieved tasks as pending and should not return them again
     * until
     ...
     * storage.
     * This should mark the retrieved tasks as pending and update their
     * expiration
     * timestamps accordingly. The specified {@link ScheduledTask} instances
     * should
     * be directly modified via {@link
     ...
     * persistent
     * storage and is no longer available for dequeue.
     *
     ...
     * the
     * {@link State} of this instance obtained from {@link #getState()} is
     * indicates
     * the service is {@linkplain State#isAvailable() available} or until there
     * are
     * no more pending or postponed tasks. If the state transitions such that
     * the
     * service is no longer {@linkplain State#isAvailable() available} the only
     * previously scheduled tasks will be handled before the processing
     * terminates.
     * This method does not return until handling of the tasks is complete.
     *
     */
     ...
     * is
     * received. This extracts the {@link TaskResult} value and traps any
     * exceptions
     * (there should be none). It records the timings from the handling and
     * calls
     * {@link #postProcess(ScheduledTask)}.
     *
     ...
     * overridden to
     * do any special handling on the {@link ScheduledTask} after it has been
     ...
     * and
     * using the specified {@link Timers} instance.
     *
     ...
     * Stat.initialize
     * the base properties and then call {@link #doInit(JsonObject)} to complete
     * the
     * configuration. This implementation will ensure that this function is
     * called
     * in the {@link State#UNINITIALIZED} and that the service transitions to
     * the
     ...
     * that
     * class using the default constructor and then initializes the constructed
     ...
     * the
     * state} to {@link State#DESTROYING}, call {@link #doDestroy()} and then
     * perform {@link #notifyAll()} and set the state to {@link
     * State#DESTROYED}.
     */
    public void destroy() {
        synchronized (this) {
    ...
    public Map<Statistic, Number> getStatistics() {
        synchronized (this.getStatisticsMonitor()) {
            ...
            statsMap.put(Stat.taskGroupFailureCount, this.getFailedTaskGroupCount());
     ...
     * handled
     * by the scheduling service. This returns <code>null</code> if no
     * non-follow-up
     ...
     * handled by the scheduling service, or <code>null</code> if no tasks
     *         have been handled.
     */
     ...
     * by
     * the scheduling service. This returns <code>null</code> if no follow-up
     * tasks
     * have been handled.
     *
     * @return The average task compression from collapsing follow-up tasks
     *         handled
     * by the scheduling service, or <code>null</code> if no follow-up tasks
     *         have been handled.
     */
    ...
    /**
     * Gets the Stat.dequeue hit ratio. This returns <code>null</code> if there have
     * been
     * no attempts to Stat.dequeue a task.
     *
     * @return The Stat.dequeue hit ratio, or <code>null</code> if no attempts have
     *         been
     *         made to Stat.dequeue a task.
     */
    ...
    /**
     * Call this to increment the number of times Stat.dequeue has been called with
     * or
     ...
     *        to
     *            be dequeued, otherwise <code>false</code> for a "miss".
     */
     ...
     *         taken
     *         from scheduling until completion, or <code>null</code> if no
     *         non-follow-up tasks have been handled.
     */
     ...
     * was
     * completed. This returns <code>null</code> if no task groups have been
     * completed.
     *
     ...
     * task was completed, or <code>null</code> if no task groups have been
     *         completed.
     */
     ...
     * {@link TaskHandler#handleTask(String, Map, int, Scheduler)} has been
     * called
     * to handle follow-up tasks to the number of times it has been called to
     * handle
     ...
     * Stat.parallelism
     * achieved. This returns <code>null</code> if no tasks have yet been
     * handled.
     *
     * @return The ratio of the total handling time across all threads to the
     *         total
     * active handling time of the task scheduler, or <code>null</code> if
    ...
    protected static class ScheduledTask {
         ...
         * storage.
         *
         * @param jsonText The serialized JSON representation of the
         *                                   follow-up task.
         * @param followUpId The optional external persistence ID for
         * the follow-up task so it can be later
         *                                   marked complete in and deleted from
         *                                   persistent storage.
         * @param multiplicity The collapsed multiplicity from persistent
         * storage, which may be one (1) if the
         * follow-up task did not allow collapsing
         *                                   with duplicate tasks.
         * @param expirationTime The millisecond UTC time since then epoch
         * when the follow-up task is considered to be
         *                                   "expired".
         * @param elapsedMillisSinceCreation The number of milliseconds that
         *        have
         * elapsed since the deserialized task was
         * originally created, or <code>null</code> if
         *                                   unknown.
         */
         ...
         * Either
         * all the tasks are follow-up tasks or all are <b>not</b> follow-up
         * tasks.
         *
         ...
         * task. If
         * this is a follow-up task then this returns <code>true</code> if the
         * follow-up
         * task is expired, otherwise <code>false</code>.
         *
         ...
         * task in
         * persistent storage. This should always return <code>null</code> if
         * {@link #isFollowUp()} is <code>false</code>. This may return
         * <code>null</code> if {@link #isFollowUp()} is <code>true</code> if
         * the
         * external persistent storage mechanism does not require an external
         * ID.
         *
         ...
         * returns the
         * remaining number of backing tasks. If no backing tasks remain then
         * this
         ...
         * ResourceKey}
         * instances identifying the resources for the backing tasks for this
         * instance.
         *
         * @return The <b>unmodifiable</b> {@link Set} containing the
         * {@link ResourceKey} instances identifying the resources for the
         *         backing tasks for this instance.
         */
         ...
         *         with the
         * backing task from this instance, and <code>false</code> if collapse
         *         is not allowed.
         */
         ...
         * This
         * returns <code>null</code> if the {@link ScheduledTask} has not yet
         * been
         * handled, otherwise it returns {@link Boolean#TRUE} or {@link
         * Boolean#FALSE}.
         *
         ...
         * locks
         * are required this simply returns <code>true</code>.
         *
