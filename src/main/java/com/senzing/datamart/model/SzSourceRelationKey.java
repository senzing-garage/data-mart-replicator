...
/**
 * The key for categorizing relations from an entity to another entity or a 
 * data source from which the related entities contain records.
 */
public class SzSourceRelationKey implements Comparable<SzSourceRelationKey> {
  /**
   ...
   *                              <code>null</code>
   */
  public SzSourceRelationKey(SzMatchType  matchType,
                             String       matchKey,
                             String       principle)                      
  ...
  /**
   * Creates the variant {@link SzSourceRelationKey} instances for the specified
   ...
   *                              <code>null</code>
   */
  public static Set<SzSourceRelationKey> variants(SzMatchType  matchType,
                                                  String       matchKey,
                                                  String       principle)
        throws NullPointerException
  {
    Set<SzSourceRelationKey> set = new TreeSet<>();
    set.add(new SzSourceRelationKey(matchType, null, null));
    set.add(new SzSourceRelationKey(matchType, matchKey, null));
    set.add(new SzSourceRelationKey(matchType, matchKey, principle));
    set.add(new SzSourceRelationKey(matchType, null, principle));
    return set;
  }
   ...
   * 
   * @param key The {@link SzSourceRelationKey} to compare with.
   * @return A negative number, zero (0) or a positive number depending
   *         on whether this instance is less-than, equal-to or greater-than
   *         the specified instance, respectively.
   */
  public int compare(SzSourceRelationKey key) {
    ...
    if (!Objects.equal(mt1, mt2)) {
        if (mt1 == null) return -1;
        if (mt2 == null) return 1;
