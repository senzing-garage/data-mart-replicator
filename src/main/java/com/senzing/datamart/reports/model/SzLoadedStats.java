...
public class SzLoadedStats implements Serializable
{
    ...
    /**
     * The total number of records loaded in the entity repository that failed
     * to match against any other record. This is also the number of entities in
     * the repository that only have a single record.
     */
    private long totalUnmatchedRecordCount = 0L;

    /**
     * The {@link SortedMap} of {@link String} data source code keys to {@link
     * SzSourceLoadedStats} values describing the count statistics for that data
     * source.
     */
     ...
     *
     * @param entityCount The total number of entities that have been loaded for
    ...
    /**
     * Gets the total number of records that have been loaded to the entity
     * repository that did <b>not</b> match against any other records. This is
     ...
     * statistics for each data source. The returned value list should contain
     * only one element for each data source.
     *
     * @return The {@link List} of {@link SzSourceLoadedStats} describing the
     *             count statistics for each data source. The returned {@link
     *             List} should contain only one element for each data source.
     */
     ...
     * statistics for each data source. This clears any existing data source
     * counts before setting with those specified. The specified {@link List}
     * should contain only one element for each data source, but if duplicates
     ...
     * statistics for a data source to the existing {@link SzSourceLoadedStats}
     * for this instance. If the specified {@link SzSourceLoadedStats} has the
     * same data source code as an existing {@link SzSourceLoadedStats} instance
     * then the specified value replaces the existing one for that data source
     * code.
     * 
