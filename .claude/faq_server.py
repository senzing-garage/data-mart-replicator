...
# dependencies = ["mcp"]
# ///
"""Data Mart Replicator FAQ MCP Server — queryable FAQ for project knowledge.
...
mcp = FastMCP(
    "data-mart-replicator-faq",
    instructions=(
        "Data Mart Replicator FAQ server. This project has complex concurrency, "
        "task scheduling, incremental report updates, and distributed locking "
        "requirements. Consult these FAQs BEFORE making design decisions, "
        "modifying task handlers, changing database operations, or touching "
        "concurrency/locking code. Critical design invariants for correctness "
        "and thread-safety are documented here that cannot be discovered by "
        "reading code alone. Key topics: INFO message parsing, task scheduling "
        "and follow-ups, entity delta computation, cross-source report updates "
        "with match key and ER code breakdowns, distributed locking, "
        "backpressure, and multi-process safety. "
